...
```{r}
# default histogram
ggplot(iris, aes(x = Sepal.Width, fill = Species)) + geom_histogram(bins = 20)

# use position = 'identity' for overlapping histograms
ggplot(iris, aes(x = Sepal.Width, fill = Species)) + geom_histogram(bins = 20, position = 'identity')

# use alpha to make the plot semi-transparent, so you can see the overlap of the histograms
ggplot(iris, aes(x = Sepal.Width, fill = Species)) + geom_histogram(binwidth = 0.1, position = 'identity', alpha = 0.5)
```
...
# ggplot doesn't give you the parameters (slope and y-intercept) of the line of 
# best fit, but you can find them with other functions like lm()
...
```{r}
ggplot(iris, aes(x = Species, y = Petal.Length, fill = Species)) + geom_boxplot() + geom_jitter()
```
...
ggplot(iris, aes(x = Species, y = Petal.Length, fill = Species)) + geom_boxplot() + geom_quasirandom(method = 'smiley')
```
...
ggplot(iris, aes(x = Sepal.Width, y = Sepal.Length, color = Petal.Width)) + geom_point()
```

**Shape**

```{r}
# can change the shape, although it's hard to see
...
```{r}
# size is not recommended for categorical variables, but you can do it
ggplot(iris, aes(x = Sepal.Width, y = Sepal.Length, size = Species)) + geom_point()

# works much better with continuous variables
ggplot(iris, aes(x = Sepal.Width, y = Sepal.Length, size = Petal.Width)) + geom_point()
```
...
# inside geom_point()
ggplot(iris, aes(x = Petal.Length, y = Petal.Width)) + geom_point(aes(color = Species)) + geom_smooth(method = 'lm')
```
...
```{r}
ggplot(iris, aes(x = Sepal.Width, y = Sepal.Length, color = Species)) + geom_point() +
# use the limits argument to set the range of the axis manually
  scale_x_continuous(name = 'sepal width (cm)', limits = c(0, 8)) +
  scale_y_continuous('sepal length (cm)', limits = c(0, 8)) +
# use the values argument to set the colors manually
  scale_color_manual('species', values = c('firebrick3', 'forestgreen', 'deepskyblue3'))
```
