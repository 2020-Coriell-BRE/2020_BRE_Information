...
## 2020-07-14 Reading Data with `readr` and Tidying/Wrangling Data with `tidyr`
...
```{r}
### don't worry about the parsing failures
...
# If you don't know where your .Rmd file is saved, you can get the location
# using getwd() ("get working directory")
getwd()
```
...
# You can see the file we wrote/saved is the same as the table we modified
measles2
measles_read
```
...
# notice that bind_cols() adds a number to columns that are repeated so that each column has a unique name
...
band_members
band_instruments
# if you don't specify the columns to join by, it will join by any columns with
# the same name and give you a message telling you what it joined by
...
# order matters; If you switch the tables, it will keep the information from 
# the first table in the function
...
full_join(band_members, band_instruments2, by = c('artist' = 'name'))
```
